          ...
          <div className="flex flex-col gap-px bg-white/[0.03]">
            {t.experience.entries.map((exp, i) => (
              <motion.div
                ...
                className="group bg-card p-6 transition-all duration-200 hover:bg-[#1a1a1a]"
              >
                <div className="mb-3 flex items-baseline justify-between gap-4">
                  <h3 className="text-sm font-medium text-foreground/80">
                    {exp.role}
                  </h3>
                  <span className="shrink-0 text-[10px] text-muted-foreground/35 tabular-nums">
                    {exp.period}
                  </span>
                </div>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  {exp.tech.map((t) => (
                    <span
                      key={t}
                      className="px-2 py-0.5 text-[10px] text-tui-cyan/60 bg-tui-cyan/[0.06] rounded-sm"
                    >
                      {t}
                    </span>
                  ))}
